...
from app.source import materialize_repo
...
def execute_scan(repo_path: str, notes: str) -> Dict[str, Any]:
    with materialize_repo(repo_path) as (repo_root, source_type, source_ref, repo_name):
        started_at = now_iso()
        repo_id = db.insert_repo(repo_name, source_type, source_ref, started_at)
        scan_id = db.insert_scan(
            repo_id,
            "running",
            started_at,
            {"candidate_paths": 0, "validated_with": "pending", "scanned_files": []},
        )

        trace(
            scan_id,
            "ingest",
            "Materialized repo source",
            {"repo_path": source_ref, "source_type": source_type, "repo_root": str(repo_root)},
        )
        files = ingest_repo(str(repo_root))
        ...
        trace(
            ...
            {"count": len(candidates), "rule_ids": [candidate["rule_id"] for candidate in candidates]},
        )
        ...
        trace(
            ...
            "validate",
            "Validated findings",
            {"count": len(validated["findings"]), "validated_with": validated["validated_with"]},
        )
        ...
        return {"scan_id": scan_id, "repo_path": source_ref}
